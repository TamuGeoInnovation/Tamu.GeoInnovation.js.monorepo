...
export class EsriMapService {
  ...
  private next(
    ...
    this.loadLayers(LayerSources.filter((l) => l.loadOnInit));
  ...
  public selectFeatures(properties: SelectFeaturesProperties) {
    ...
    // Source object
    const source = Object.assign(LayerSources.find((src) => src.id == 'selection-layer'));

    // Add the symbol and polygon type to each feature
    const features = graphics.map((ft) => {
      const feature = ft;
      feature.symbol = SelectionSymbols[ft.geometry.type];
