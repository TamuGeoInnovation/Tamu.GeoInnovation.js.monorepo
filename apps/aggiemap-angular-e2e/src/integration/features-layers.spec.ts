describe('Correct Layers', () => {
  before(() => {
    cy.visit('https://aggiemap.tamu.edu/map/d')
      cy.get('tamu-gisc-layer-list > .sidebar-component-content-container')
        .within(($list) => {
          cy.get('p').each(($el, index) => {
            cy.wrap($el).as('element')
            cy.get('@element')
              .should('be.visible', {timeout:2000}) 
              if(index === 0) {

              } else if (index === 7) {

              } else {
                cy.get('@element').click()
                cy.server()
                cy.route('GET', '*')
                  .as('apiCheck')
                cy.wait('@apiCheck').then((interception) => {
                  assert.isNotNull(interception.response.body, 'API call has data')
                })
              }
          })
        })
  })

  it('Construction Zone', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(1)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Construction Zone')
    cy.get('.sidebar-component-content-container > :nth-child(9)')
      .should('be.visible')
      .and('contain', 'Construction Area')
    cy.get(':nth-child(9) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAUCAYAAACNiR0NAAAAAXNSR0IB2cksfwAAAAlwSFlzAAAOxAAADsQBlSsOGwAAAENJREFUOI1jYaAyYKGZgf9DGf5TahjjagZGFnQBcg2DOYgFmyAlgLYupAYYYgaORsogNHA0Uig3kJIwhAUXC7oApQAAQ8kZ9+L+/N4AAAAASUVORK5CYII=')
      .and('have.attr', 'alt', 'Construction Area')
  })
  it('Points of Interest', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(2)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Points of Interest')
    cy.get('.sidebar-component-content-container > :nth-child(10)')
      .should('be.visible')
      .and('contain', 'Points of Interest')
    cy.get(':nth-child(10) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Points of Interest')
  })
  it('Restrooms', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(3)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Restrooms')
    cy.get('.sidebar-component-content-container > :nth-child(11)')
      .should('be.visible')
      .and('contain', 'Unisex Restrooms')
    cy.get(':nth-child(11) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Unisex Restrooms')
  })
  it('Lactation Rooms', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(4)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Lactation Rooms')
    cy.get('.sidebar-component-content-container > :nth-child(12)')
      .should('be.visible')
      .and('contain', 'Lactation Rooms')
    cy.get(':nth-child(12) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Lactation Rooms')
  })
  it('Visitor Parking', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(5)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Visitor Parking')
    cy.get('.sidebar-component-content-container > :nth-child(13)')
      .should('be.visible')
      .and('contain', 'Visitor Parking')
    cy.get(':nth-child(13) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Visitor Parking')
  })
  it('Accessible Entrances', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(6)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Accessible Entrances')
    cy.get('.sidebar-component-content-container > :nth-child(14)')
      .should('be.visible')
      .and('contain', 'Assisted Open Entrance')
    cy.get(':nth-child(14) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Assisted Open Entrance')
  })
  it('Emergency Phones', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(7)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Emergency Phone')
    cy.get('.sidebar-component-content-container > :nth-child(15)')
      .should('be.visible')
      .and('contain', 'Emergency Phone')
    cy.get(':nth-child(15) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Emergency Phone')
  })
  it('Physical Distance Study Area', () => {
    cy.get('tamu-gisc-layer-list > .sidebar-component-content-container > :nth-child(8)')
      .should('have.class', 'layer-item ng-star-inserted loaded')
      .and('contain', 'Physical Distance Study Area')
    cy.get('.sidebar-component-content-container > :nth-child(16)')
      .should('be.visible')
      .and('contain', 'Physical Distance Study Area')
    cy.get(':nth-child(16) > .image-container > img')
      .should('have.attr', 'src', 'data:image/png;base64,')
      .and('have.attr', 'alt', 'Physical Distance Study Area')
  })
})