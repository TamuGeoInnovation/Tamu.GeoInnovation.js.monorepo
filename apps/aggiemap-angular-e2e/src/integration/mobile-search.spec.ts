...
mobileSizes.forEach((size) => {
  describe(`Test Different Route Options on Mobile Layers Page: ${size} resolution`, function () {
    ...
    it('Input Building Search', () => {
      cy.get('tamu-gisc-search-mobile').should('be.visible').click();
    ...
    it('Check Popup Results', () => {
      ...
      cy.get('.feature-style-1').should('not.be.empty').and('be.visible');
      cy.get('.feature-style-2').should('not.be.empty').and('be.visible');
      cy.get('.feature-style-2 > :nth-child(1)').should('not.be.empty').and('be.visible');
      cy.get('.feature-style-2 > :nth-child(2)').should('not.be.empty').and('be.visible');
      cy.get('.feature-style-2 > :nth-child(3)').should('not.be.empty').and('be.visible');

      // check if correct copy URL is displayed
      cy.get('tamu-gisc-copy-field').should('be.visible').and('contain.text', 'https://aggiemap.tamu.edu/?bldg=0446');
      cy.wait(2000);
    });

    it('Check Walk Route Directions', () => {
      cy.wait(2000);
      cy.contains('Directions To Here').click();
      ...
      cy.get('.active > .eta').should('contain.text', '51').should('be.visible');
      cy.get(':nth-child(1) > .quantity').should('contain.text', '51').should('be.visible');
      cy.get(':nth-child(3) > .quantity').should('contain.text', '999').should('be.visible');
      cy.get(
        ':nth-child(2) > tamu-gisc-trip-planner-mode-switch > :nth-child(2) > :nth-child(1) > .directions-container > :nth-child(1) > p'
      )
        .should('include.text', 'Test Direction 2')
        .should('be.visible');
      cy.get(':nth-child(2) > p').should('include.text', 'Test Direction 4').should('be.visible');
      cy.get('.directions-container > :nth-child(3) > p').should('include.text', 'Test Direction 5').should('be.visible');
    ...
    it('Check Bike Route Directions', () => {
      ...
      cy.get('.travel-modes > :nth-child(2) > div').click().should('have.class', 'active');
      cy.get('.handle').move({ x: 0, y: -400, force: true }); // drag popup back into view
      cy.get('.directions-overview').should('be.visible');
      cy.get('.directions-container').should('be.visible');
      cy.get('.active > .eta').should('contain.text', '51').should('be.visible');
      cy.get(':nth-child(1) > .quantity').should('contain.text', '51').should('be.visible');
      cy.get(':nth-child(3) > .quantity').should('contain.text', '888').should('be.visible');
      cy.get(':nth-child(2) > p').should('include.text', 'Test Direction 2').should('be.visible');
      cy.get('.directions-container > :nth-child(3) > p').should('include.text', 'Test Direction 4').should('be.visible');
      cy.get(':nth-child(4) > p').should('include.text', 'Test Direction 5').should('be.visible');
      cy.get('.handle').move({ x: 0, y: 400, force: true }); // drag popup back down to access route options again
    });
    ...
    it('Check Car Route Directions', () => {
      ...
      cy.get('tamu-gisc-trip-planner-mode-picker-mobile').should('be.visible');
      cy.get('.travel-modes > :nth-child(3) > div').click().should('have.class', 'active');
      cy.get('.handle').move({ x: 0, y: -400, force: true }); // drag popup back into view
      cy.get('.directions-overview').should('be.visible');
      cy.get('.directions-container').should('be.visible');
      cy.get('.active > .eta').should('contain.text', '51').should('be.visible');
      cy.get(':nth-child(1) > .quantity').should('contain.text', '51').should('be.visible');
      cy.get(':nth-child(3) > .quantity').should('contain.text', '777').should('be.visible');
      cy.get(
        ':nth-child(2) > tamu-gisc-trip-planner-mode-switch > :nth-child(2) > :nth-child(1) > .directions-container > :nth-child(1) > p'
      )
        .should('include.text', 'Test Direction 2')
    ...
    it('Check Bus Route Directions', () => {
      ...
      cy.get('tamu-gisc-trip-planner-mode-picker-mobile').should('be.visible');
      cy.get('.travel-modes > :nth-child(4) > div').click().should('have.class', 'active');
      cy.get('.handle').move({ x: 0, y: -400, force: true }); // drag popup back into view
      cy.get('.directions-overview').should('be.visible');
      cy.get('.directions-container').should('be.visible');
      cy.get('.active > .eta').should('contain.text', '25').should('be.visible');
      cy.get(':nth-child(1) > .quantity').should('contain.text', '25').should('be.visible');
      cy.get(':nth-child(3) > .quantity').should('contain.text', '555').should('be.visible');
      cy.get(':nth-child(2) > p').should('include.text', 'Test Direction 2').should('be.visible');
      cy.get('.directions-container > :nth-child(3) > p').should('include.text', 'Test Direction 4').should('be.visible');
      cy.get(':nth-child(4) > p').should('include.text', 'Test Direction 5').should('be.visible');
