  ...
  describe(`Routing Options: ${size} Resolution`, () => {
    ...
    it('Option Description', () => {
      cy.contains(
        ...
        'Will attempt to calculate a route with accessible access such as ramps, doors, etc.'
      );
    });
    it('Test Checkbox', () => {
      cy.get('tamu-gisc-checkbox').as('option');
      cy.get('@option').find('i').as('checkBox').should('have.class', 'material-icons');
      cy.get('@option').click();
      cy.get('@checkBox').should('have.class', 'material-icons active');
    });
  });
});
