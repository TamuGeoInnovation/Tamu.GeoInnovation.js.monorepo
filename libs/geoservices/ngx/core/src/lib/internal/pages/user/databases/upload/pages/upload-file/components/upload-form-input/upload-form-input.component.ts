...
export class UploadFormInputComponent extends FileComponent implements OnInit {
  ...
  public ngOnInit() {
    this.dragOver = merge(
      fromEvent<DragEvent>(this.dragAreaRef.nativeElement, 'dragover'),
      fromEvent<DragEvent>(this.dragAreaRef.nativeElement, 'dragleave'),
      fromEvent<DragEvent>(this.dragAreaRef.nativeElement, 'drop')
    ).pipe(
      switchMap((event) => {
