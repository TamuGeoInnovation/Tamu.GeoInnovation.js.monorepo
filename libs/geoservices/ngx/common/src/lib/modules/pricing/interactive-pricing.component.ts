...
import { AuthService } from '@tamu-gisc/geoservices/ngx/data-access';
...
export class InteractivePricingComponent implements OnInit {
  ...
  public selectedTierCost: Observable<number | null | undefined>;
  ...
  public recurringInterval: Observable<IntervalOption | undefined>;
  ...
  public ngOnInit(): void {
    ...
    this.frequencyType = (this.form.get('frequency') as AbstractControl).valueChanges.pipe(
    ...
    this.partnerPricing = (this.form.get('partnerProgram') as AbstractControl).valueChanges.pipe(
      startWith(this.form.get('partnerProgram')?.value),
      shareReplay()
    );
    ...
    this.selectedCreditTier = combineLatest([
      (this.form.get('creditCount') as AbstractControl).valueChanges.pipe(startWith(this.form.get('creditCount')?.value)),
    ...
    this.isSLA = (this.form.get('sla') as AbstractControl).valueChanges.pipe(
    ...
    this.ctaText = combineLatest([this.recurringInterval, this.isSLA]).pipe(
      map(([interval, sla]) => {
        // return text for each frequency type and special case for when SLA is selected
